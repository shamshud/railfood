    ...
    <header class="masthead text-white text-center">
      ...
      <div class="container">
        <div class="row">
          ...
          if(isset($_GET['m'])){
            if($_GET['m']=='upds'){
              echo '<h5>Updated Successfully</h5>';
            }
          }
          ?>
          </div>
...
<?php
$q=$mysqli->query("select * from orders where status='Received' order by id asc");
$i=0;
while($qu=mysqli_fetch_array($q)){
  ...
  echo '<tr>
        ...
        <td>'.$qu['qty'].'</td>
        <td>'.$qu['status'].'</td>
        <td><a class="btn btn-info" href="ins.php?upd=Delivered&id='.$id.'">Delivered</a></td>';
